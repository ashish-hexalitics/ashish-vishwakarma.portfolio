...
import { projects, GsapAnimation, Iprojects } from "../constant/index";
...
const ProjectsSection: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      <div className="text-xl font-bold text-center mb-10 space-x-4">
        {[
          { label: "Projects", value: "projects" },
          { label: "Gsap Animation", value: "GsapAnimation" },
        ].map((tab, index) => {
          return (
            <label
              className={`text-gray-600 hover:text-gray-900 hover:underline ${
                tab.value === projectTab ? "underline" : ""
              }`}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 px-6 md:px-10 relative z-10">
        {(projectTab === "projects" ? projects : GsapAnimation).map(
          (project: Iprojects, index) => (
            <div
              key={index}
